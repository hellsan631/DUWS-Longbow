...
if (fobpos distance player < 100) exitWith {_fobFound=true}; // leave the foreach loop with
...
if ((!_fobFound) && ((hq_blu1 distance player)>100)) exitWith {["veh_refit_hint",["Too far away","Your vehicle must be near the base or an FOB"]] call bis_fnc_showNotification};
